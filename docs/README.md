...
3. Choose the branch you want to publish (e.g., `miniapp` or `main` after merging) and set the **folder** to `/docs`.
4. Save. GitHub Pages will publish at `https://<your-username>.github.io/<repo-name>/docs/` once the build finishes.

## Opening the mini app in Telegram
1. Make sure your bot has a domain that matches the GitHub Pages URL (e.g., `https://<username>.github.io/<repo>/docs/`). For public Pages, Telegram accepts the `github.io` domain.
2. In **BotFather**:
...
Open `/docs/index.html` directly (e.g., via GitHub Pages). The app detects when Telegram’s SDK is unavailable and runs in standalone mode automatically.
